...
# #CREATING LOG FILES
my $std_out = '/home/modupe/.LOG/KaKs-'.`date +%m-%d-%y_%T`; chomp $std_out; $std_out = $std_out.'.log';
...
open(STDOUT, '>>', "$std_out.ref") or die "Log file doesn't exist";
open(STDERR, '>>', "$std_out.ref") or die "Error file doesn't exist";
ENDREFPERL
...
while ($addi<$sum) {
  ...
  foreach my $genename (sort keys %GENE ){
    ...
    foreach my $number (1..$GENE{$genename}){
      $addi++;
      my $extractsam ="~/.software/samtools-0.1.19/samtools mpileup -uf $ref aln.sorted.bam -r $CHR{$genename}{$number}:$POSstart{$genename}{$number}-$POSend{$genename}{$number} | bcftools view -cg - | /home/modupe/.software/samtools-0.1.19/bcftools/vcfutils.pl vcf2fq > $genename-$number.pep";
...
use threads;
use Thread::Queue;
#open(STDOUT, '>>', "$std_out.combine") or die "Log file doesn't exist";
#open(STDERR, '>>', "$std_out.combine") or die "Error file doesn't exist";
my \$std_out = "$std_out.kaks";
ENDCOMBINE
...
use threads;
use Thread::Queue;
open(STDOUT, '>>', "$std_out.ext") or die "Log file doesn't exist";
open(STDERR, '>>', "$std_out.ext") or die "Error file doesn't exist";
ENDEXTRACT
...
`perl combine.pl`;
print "Job completed: combine.pl\nJob execution: extractkaks.pl\n";
`perl extractkaksratios.pl`;
print "Job completed: extractkaksratios.pl\nJob execution: cleanup.pl\n";
`perl cleanup.pl`;
